# Open Skills Lite：六项分析与结果交付技能

源码可见、有限授权的 Lite 版。本项目使用自定义限制性许可，**不是标准开源许可项目**。
...
Lite 1.0.0 提供六个独立 ZIP。只下载需要的技能，不必安装整套；各项技能都能读取用户已有资料，没有必须先安装另一项技能的要求。
...
本系列覆盖上述六项分析与结果交付任务，不提供客户自动分群、预测、独立因素分析、异常检测或行动管理系统。简报和诊断可以提出核查建议，但不自动实施处置。
